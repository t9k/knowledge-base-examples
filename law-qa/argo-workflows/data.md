...
注意到所有法律的文档都是高度结构化的，包含编、章、节、条四级结构（或其中的一部分），因此可以统一处理，导入到同一个 Collection。按类型导入到多个 Collection 可能会使得工具的定义和调用的逻辑变得更加复杂。如果有行业规范或公司章程，可以考虑导入到另一个 Collection 中。

**案例案件**

案例案件包含民事案件、刑事案件、执行案件、非诉保全审查案件、管辖案件等，来源为中国裁判文书网。构建案例案件数据集时，也应先划定哪些类型的案件需要被导入向量数据库。
