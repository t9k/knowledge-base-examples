...
def check_file_needs_processing(csv_file, last_modified, col, parent_col):
    ...
    search_results = col.query(expr=expr, output_fields=["case_id"], limit=1)
    ...
    exact_match_results = col.query(expr=expr,
    ...
    delete_by_source(filename, col, parent_col)
    return True


def delete_by_source(source, col, parent_col):
    ...
    try:
        ...
        result = col.delete(expr=expr)
        col.flush()
        logger.info(
            f"Deleted {result['delete_count']} records with source: {source}")
        result = parent_col.delete(expr=expr)
        parent_col.flush()
        logger.info(
            f"Deleted {result['delete_count']} records with source: {source}")
...
def extract_metadata_with_llm(chunk, client):
    system_prompt = "你是一名法律信息提取助手，擅长从案件描述中提取关键元素。直接给出最终的提取结果。"

    # 定义所有prompt模板
    prompts = {
        "dates":
        f"""你的任务是从裁判文书片段中提取所有出现的日期。你的回答应为一个字符串，其中包含所有日期，用换行符分隔：
...
{chunk}""",
        "locations":
        f"""你的任务是从裁判文书片段中提取所有出现的地点。你的回答应为一个字符串，其中包含所有地点，用换行符分隔：
...
{chunk}""",
        "people":
        f"""你的任务是从裁判文书片段中提取所有出现的人物（自然人）。你的回答应为一个字符串，其中包含所有人物的姓名、职业和在当次审判中的角色，人物之间用换行符分隔，姓名、职业、角色之间用半角分号分隔：
...
{chunk}""",
        "numbers":
        f"""你的任务是从裁判文书片段中提取所有出现的数额。你的回答应为一个字符串，其中包含所有数额，用换行符分隔：
...
{chunk}""",
        "parties_llm":
        f"""你的任务是从裁判文书片段中提取所有出现的当事人。你的回答应为一个字符串，其中包含原告（上诉人）、被告（被上诉人）、原审原告、原审被告，用换行符分隔：
...
{chunk}"""
    ...
    if len(results["dates"]) > 500 or re.search(r'[\u4e00-\u9fff]', results["dates"]):
        results["dates"] = "<none>"
    ...
    if len(results["numbers"]) > 400 or re.search(r'[\u4e00-\u9fff]{5}', results["numbers"]):
...
def setup_milvus_collection(dense_dim):
        ...
        if utility.has_collection(PARENT_COLLECTION_NAME):
            ...
            parent_fields = [
                FieldSchema(name="parent_id",
                            ...
                            is_primary=True,
                            max_length=36),
                FieldSchema(name="case_id",
                            dtype=DataType.VARCHAR,
                            max_length=36),
                FieldSchema(name="chunk",
                            dtype=DataType.VARCHAR,
                            max_length=30000),
    ...
    if utility.has_collection(COLLECTION_NAME):
        ...
        fields = [
            ...
            FieldSchema(name="case_number",
                        dtype=DataType.VARCHAR,
                        max_length=200),
            FieldSchema(name="case_name",
                        dtype=DataType.VARCHAR,
                        max_length=500),
...
def record_generator(csv_path, chunksize, last_modified):
    """
...
def insert_batch(col, buffer, ef, parent_col):
    ...
    to_insert = [
        ...
        [r["source"] for r in buffer],
        [r["last_modified"] for r in buffer],
        sparse_embeddings,
        dense_embeddings,
    ]
    if parent_col:
...
def insert_parent_batch(parent_col, parent_buffer):
    ...
    parent_col.insert([
        ...
        [r["source"] for r in parent_buffer],  # source
...
def main():
    ...
    llm_client = OpenAI(base_url=CHAT_BASE_URL, api_key="dummy", timeout=300)
    ...
    for csv_file in csv_files:
        last_modified = datetime.datetime.fromtimestamp(
            os.path.getmtime(csv_file)).isoformat()
        if not check_file_needs_processing(csv_file, last_modified, col,
                                           parent_col):
            continue

        print(f"Processing file: {csv_file}")
        insert_data_streaming(
            col, parent_col, record_generator(csv_file, 100000, last_modified),
