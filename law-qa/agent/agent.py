...
def run_cli(bot, tokenizer_path, max_tokens):
    """
    Runs the command-line interface for the chat bot.

    Args:
        bot: The Assistant instance.
        tokenizer_path: Path to the tokenizer.
        max_tokens: The maximum number of tokens for the chat history.
    """
    # Initialize the tokenizer (e.g., Qwen3-32B)
    try:
    ...
    user_prompt_prefix = ("提示：法小助必须精准识别用户的真实意图；需要调用检索工具进行查询，除非答案已经在上下文中；如果用户没有提及法条编号，不得使用法条编号来查询或检索；"
    ...
    while True:
        ...
        if query.strip() == '/history':
            ...
            continue  # Do not send '/history' to the model

        if query.strip() == '/clear':
            messages = []
            print('Chat history cleared!\n')
            continue  # Do not send '/clear' to the model

        # Append the user query to the chat history.
        if messages:
            query = user_prompt_prefix + query
        messages.append({'role': 'user', 'content': query})
        messages = truncate_messages(messages, tokenizer, max_tokens)
        ...
        # Append function calls and the final answer to the chat history.
        for r in response:
...
def count_tokens(messages, tokenizer):
    """
    Calculates the total number of tokens for all content fields in a list of messages.
    ...
    for msg in messages:
        ...
        if content:
            ...
            total_tokens += 4  # Approximation for message overhead
        elif function_call:
            texts.append(json.dumps(function_call, ensure_ascii=False))
            total_tokens += 8  # Approximation for function call overhead
        else:
            pass

    text = ''.join(texts)
    return len(tokenizer.encode(text)) + total_tokens


def truncate_messages(messages, tokenizer, max_tokens):
    """
    Removes the oldest messages until the total token count is within the max_tokens limit.
    ...
    """
    while count_tokens(messages, tokenizer) > max_tokens:
...
def main():
    """
    Main function to set up and run the legal assistant bot.
    It handles command-line arguments for configuration,
    initializes the language model and tools, and starts
    either the web UI or a command-line chat session.
    """
    parser = argparse.ArgumentParser(
        description='A professional legal intelligent assistant.')
    parser.add_argument('--webui',
                        action='store_true',
                        help='Run with the WebUI interface.')
    parser.add_argument('--law-searcher',
                        action='store_true',
                        help='Enable the law-searcher MCP server.')
    parser.add_argument('--case-searcher',
                        action='store_true',
                        help='Enable the case-searcher MCP server.')
    parser.add_argument('--reranker',
                        action='store_true',
                        help='Enable the reranker MCP server.')
    ...
    parser.add_argument(
        ...
        help='The endpoint URL for the model server.',
    )
    parser.add_argument(
        '--tokenizer-path',
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=10000,
        help='The maximum number of tokens in the chat history.',
    )
    args = parser.parse_args()

    # Define LLM configuration from command-line arguments
    llm_cfg = {
        'model': args.model,
        # Use a custom endpoint compatible with the OpenAI API:
        'model_server': args.model_server,  # api_base
        'api_key': 'EMPTY',
        'generate_cfg': {
            'temperature': 0.0,
            'top_p': 0.95,
    ...
    # Define Tools based on command-line arguments
    mcp_servers = {}
    if args.law_searcher:
        mcp_servers['law-searcher'] = {
            'type': 'streamable-http',
            'url': 'https://home.qy.t9kcloud.cn/mcp/law-searcher/mcp/',
        }
    if args.case_searcher:
        mcp_servers['case-searcher'] = {
            'type': 'streamable-http',
            'url': 'https://home.qy.t9kcloud.cn/mcp/case-searcher/mcp/',
        }
    if args.reranker:
        mcp_servers['reranker'] = {
            'type': 'streamable-http',
            'url': 'https://home.qy.t9kcloud.cn/mcp/reranker/mcp/',
        }
    ...
    system_prompt = f"""本助手名为法小助，由 TensorStack AI 开发。
...
法小助避免写清单式内容，若必须列出，会重点强调关键信息而非追求全面。若可用 1~3 句话或一小段话回答，就会这样做。若可用自然语言列出几个逗号分隔的例子，也会优先采用这种方式。法小助关注精炼高质量的表达，而非数量。"""
    ...
    # Run the WebUI if the --webui argument is provided
    if args.webui:
        chatbot_config = {'agent.avatar': './chatbot.png'}
        WebUI(bot, chatbot_config=chatbot_config).run()
    else:
        # Run the command-line interface
        run_cli(bot, args.tokenizer_path, args.max_tokens)
