...
cat /workspace/dify_docs.json | jq -r '.data[] | .name + " " + .id' | while read -r doc_info; do
  doc_name=$(echo "$doc_info" | cut -d' ' -f1)
  doc_id=$(echo "$doc_info" | cut -d' ' -f2)
  
  # Check if file exists in workspace
  if ! grep -q "^${doc_name} " /workspace/s3_files.txt; then
...
if [ "${MODIFIED_ONLY}" = "true" ] && [ -f /workspace/modified_files.txt ]; then
  echo "Processing only modified files..."
  files_to_process="/workspace/modified_files.txt"
else
