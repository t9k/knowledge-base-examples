...
if [ -f ${WORKSPACE_DIR}/deleted_files.txt ]; then
  cat ${WORKSPACE_DIR}/deleted_files.txt | while read -r filename; do
    if [ ! -z "$filename" ]; then
...
if [ "${ALWAY_PUSH_ALL_FILES}" = "true" ]; then
  ...
  cat ${WORKSPACE_DIR}/created_files.txt ${WORKSPACE_DIR}/modified_files.txt > ${WORKSPACE_DIR}/files_to_process.txt
...
while read -r line; do
  if [ ! -z "$line" ]; then
    ...
    is_created=$(grep -q "^${filename}$" ${WORKSPACE_DIR}/created_files.txt && echo "yes" || echo "no")
    ...
    if [ -z "$doc_id" ] || [ "$is_created" = "yes" ]; then
      ...
      is_modified=$(grep -q "^${filename}$" ${WORKSPACE_DIR}/modified_files.txt && echo "yes" || echo "no")
