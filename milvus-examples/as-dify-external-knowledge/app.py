...
def perform_search(collection,
    ...
    if search_mode == "hybrid":
        ...
        # 创建密集向量搜索请求
        dense_req = AnnSearchRequest([query_embeddings["dense"][0]],
        ...
        # 创建稀疏向量搜索请求
        sparse_req = AnnSearchRequest([query_embeddings["sparse"][[0]]],
        ...
        results = collection.search(data=[dense_vector],
        ...
        results = collection.search(data=[sparse_vector],
...
def retrieval():
    try:
        ...
        try:
            ...
            # 需要输出的字段
            output_fields = [
                "fact", "relevant_articles", "accusation", "punish_of_money",
                "criminals", "imprisonment", "life_imprisonment",
                "death_penalty"
            ]
            ...
            try:
                ...
                for hit in results:
                    ...
                    if search_mode == "hybrid":
                        score = hit.score / (
                            2 /
                            (MILVUS_CONFIG["hybrid_search"]["rrf_k"] +
                             1))  # 归一化到0-1范围
                    else:
                        # 根据搜索模式和度量类型转换分数
